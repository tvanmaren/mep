...
v0.1 ships a **credible FOSS product** (unix substrate, executor plugs, workflow closure, transitional routing). v0.2 closes the **architecture gap** between that shipping path and the vision: composed explicit state as the default routing input, forensic inference as an opt-in layer, and a single mode-aware answer from `where`.

**Staffing rule:** keep this outline; open a v0.2 epic and shortcut stories only after v0.1 is **shippable** (see gates below). Slice titles and order may change once v0.1 contracts land.

---

## Why v0.2 exists

v0.1 intentionally leaves routing on a **transitional model** (central index + git heuristics in `resolver.sh`). That unblocks litmus, golden fixtures, and document authoring — but it is the source of recurring resolver churn and split-brain between `where` and `lifecycle status`.

v0.2 is not feature sprawl. It is the **structural fix** the vision proposal describes:
...
## Entry gates (before v0.2 epic is staffed)

All required:

| Gate | v0.1 milestone | Why v0.2 waits |
|------|----------------|----------------|
| Unix litmus green | iter 4 | infer/doctor must speak the same JSON + exit contract |
| `executionRequest` on `where --json` | iter 2 | evidence-gated autopilot commit needs the packet |
| Golden matrix (transitional) | iter 5 | v0.2 **replaces** hot-path heuristics; needs a baseline to regress against |
| At least one mode workflow closed | iter 6 **or** 7 | unified router must route real manual/autopilot steps |
| v0.1.0 tagged | iter 14 | semver + standalone repo home for v0.2 work |

Recommended before **v0.2.0** (document state) ships:
...
## Guardrail on v0.1 (apply now)

**Iteration 5 (resolver totality)** locks routing for the **v0.1 transitional model** only.
...
v0.2 **demotes** git reconstruction from the default `where` path and moves it to `infer`. Iter 5 must not make that migration harder.

---

## Release shape (proposed)

| Tag | Theme |
|-----|--------|
| **v0.1.0** | FOSS extract, unix foundation, workflow CLI, transitional resolver, manifest OK |
| **v0.2.0** | Document-colocated workflow state; runtime **composes** from prep-tree nodes |
...
**B0. Classification feedback ledger** *(v0.2.x, observability only)*  
Log finish-map / gate decisions with human verdict (`correct` | `should_have_been_X`). History surface only — does **not** write `authorshipMode`. Seeds the v0.3 classifier; pilot metric starts in v0.1 iter 13.

### Milestone C — total router (v0.2.2)
...
B milestones can start fixture work **in parallel** with late A1 once prep-tree walk API is sketched — but **C1 must not ship** until B3 proves desync does not fall through to guess routing. **E milestones do not start** until v0.2 is done.
...
## v0.3 — outline stub (not staffed)

**Status:** deferred until v0.2 success criteria pass. Staff a v0.3 epic after v0.2.0+ is shippable.

v0.3 closes the **policy classifier** gap: today the operator sets `authorshipMode`; finish-map classifies mechanical vs decision-bearing at slice planning (executor/LLM). v0.3 adds MEP **assigning or recommending** delegation policy from "does this need human judgment?" — a new operator contract, semver **0.3.0** minimum if it writes mode without explicit `/mep mode`.

**Not the same as:**

| layer | v0.1–v0.2 | v0.3 |
|-------|-----------|------|
...
### Milestone E — mode classifier (v0.3.0)
...
**Prerequisites (v0.2):** C1 unified router · enforced finish-map (v0.1 iter 8) · B0 classification feedback · at least one closed manual/default/autopilot workflow.

**Explicit non-goals for v0.3 (initial):** ownership lifecycle promotion (manual→autopilot over time without human ratification); deterministic non-LLM classifier as sole authority; fourth authorship mode.

---

## Success criteria (v0.2 program done)
...
| `wiki/prep/mep-runtime-productization/` | Parallel rewrite / contract freeze (not v0.2 scope) |
