...
| I19 | which lib file owns frontmatter primitives | **closed** in `09f8e54` — `tools/mep/lib/document.sh`; keep as history, not a live fanout |
...
| 17 close | `09f8e54` extract persistence into `document.sh`; unix-contract sources it before resolver | confirmed C12; I19 closed. **did not** promote a module-path C* or staff A2. goldens/unix-contract/stranger/litmus unchanged. `--fix` wrote brief revision `b02b97e` + status committed + implementation/checkpoint `09f8e54` |
| 17 graduate | operator 2026-09-10: 01 success #3 still needs a live driver (D0); not a tiny evidence slice. A2+ is a new v0.3 initiative | this slug ends at 17. no iter 18.
