...
`manifest.json` read-only; `status --compact --json` reports `authority: "legacy_import"`. Reads and routing work; every
writer refuses rather than no-opping — `checkpoint` and `doctor` with a `legacy_state_read_only`
finding, `mode set` with a `legacy_state_read_only` reason, all of them `blocked` at exit 2.
`mep migrate <slug> --json` writes modeled facts into frontmatter, names retired keys, and is the supported exit.
